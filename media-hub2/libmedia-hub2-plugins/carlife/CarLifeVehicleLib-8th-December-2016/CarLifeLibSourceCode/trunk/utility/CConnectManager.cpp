...
bool CConnectManager::createCmdSocket(){
	...
	if(cmdSocket->connectToServer()){
		...
		CCarLifeLog::carLifeLogLnWithPrefix("CConnectManager::createCmdSocket() error!!!!!");
		delete cmdSocket;
		cmdSocket=NULL;
		return false;
	}
...
//		ttsSocket->setConnectStatus(false);
		CCarLifeLog::carLifeLogLnWithPrefix("[readTTSData]socket.recv is failed!");
		return false;
		}
...
bool CConnectManager::readVRData(u8*data, u32 len){
	if(NULL==vrSocket){
		CCarLifeLog::carLifeLogLnWithPrefix("[readVRData]vrSocket is null!");
		return false;
		}
	
	if(!vrSocket->iSocket->recv(data,  len)){
//		vrSocket->setConnectStatus(false);
		CCarLifeLog::carLifeLogLnWithPrefix("[readVRData]socket.recv is failed!");
		return false;
		}
...
bool CConnectManager::writeVRData(u8*data, u32 len){
	if(NULL==vrSocket){
		CCarLifeLog::carLifeLogLnWithPrefix("[writeVRData]vrSocket is null!");
		return false;
		}
	
	if(!vrSocket->iSocket->send(data,  len)){
//		vrSocket->setConnectStatus(false);
		CCarLifeLog::carLifeLogLnWithPrefix("[writeVRData]socket.send is failed!");
		return false;
		}
